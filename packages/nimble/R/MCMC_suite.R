...
#' This default behavior can be altered via a variety of arguments.
...
#' Each element may be the character name of an existing R function (possibly user-defined) which acts on a numeric vector and returns a scalar (e.g., \code{mean} or \code{sd}),
...
#' Anything else will be interpreted as NIMBLE MCMC algorithms, and must have associated entries in the \code{MCMCdefs} argument.
#' Default value is \code{c('jags', 'nimble', 'nimble_RW', 'nimble_slice', 'autoBlock')}.
#' 
#' @param MCMCdefs A named list of MCMC definitions.  The names of list elements should correspond to any custom MCMC algorithms specified in the \code{MCMCs} argument.
...
#' specifying the corresponding MCMC algorithm; in particular, setting the appropriate samplers.  The code may assume existence of the R model object \code{Rmodel},
...
#' This argument will be passed directly to the \code{bugs(...)} call, from the R2WinBUGS library.
...
#' In addition, the Stan data file (\code{modelName.data.R}) must also reside in the same directory as the Stan model file.
#' Optionally, the Stan initial values file (\code{modelName.init.R}) may also be in this same directory; it will be used if present.
#' 
...
#' \code{samples}: A 3-dimensional array containing samples from each MCMC algorithm.
#' \code{summary}: A 3-dimensional array containing summary statistics for each variable and algorithm.
#' \code{timing}: A numeric vector containing timing information.
#' \code{efficiency}: Minimum and mean sampling efficiencies for each algorithm (only provided if option \code{calculateEfficiency = TRUE)}.
#' See the NIMBLE User Manual for more information about the organization of the return object.
#'  
#' @examples
#' code <- nimbleCode({
