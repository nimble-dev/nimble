...
#' 
#' This sampler updates latent multinomial distributions, using Metropolis-Hastings proposals to move observations between pairs of categories.  Each proposal moves a single observation from one category to another category, and acceptance or rejection follows standard Metropolis-Hastings theory.  The RW_multinomial sampler can make multiple attempts at transitions on each sampling iteration, which is govered by the 'tries' control argument.
#'  
#' The \code{RW_multinomial} sampler accepts the following control list elements:
#' \itemize{
#' \item tries.  An integer argument, specifying the number of independent Metropolis-Hastings proposals (and subsequent acceptance or rejection) that are attempted each time the sampler operates.  For example, if 'tries' is one, then a single proposal (of moving one observation to a different category) will be made, and either accepted or rejected.  If tries is two, this process is repeated twice.  The default value for 'tries' scales as the cube root of the total number of observations comprising the target multinomial distribution (given by the 'size' parameter of the distribution).
#' }
#'
