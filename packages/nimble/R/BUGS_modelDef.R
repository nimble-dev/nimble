...
modelDefClass$methods(setupModel = function(code, constants, dimensions, inits, data, userEnv, debug = FALSE) {
    ...
    if(nimbleOptions("enableModelMacros")){
      ...
      assignMacroInits(macroOutput$modelInfo$inits) # save inits generated by macros
      inits <- addMacroInits(inits, macroOutput$modelInfo$inits) # update full list of inits
      assignMacroParameters(macroOutput$modelInfo$parameters) # sets field macroParameters containing list of parameters macros generated
    } else {
