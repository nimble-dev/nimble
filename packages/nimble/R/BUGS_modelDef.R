...
modelDefClass$methods(processBoundsAndTruncation = function() {
    ...
    for(i in seq_along(declInfo)) {
        ...
        if(!(callName %in% c("T", "I"))) {
            ...
            truncated <- TRUE
            if(callName == "I")
                message("  [Note] Interpreting I(,) as truncation (equivalent to T(,)) in ", deparse(BUGSdecl$code, maxlen =1), "; this is only valid when ", deparse(BUGSdecl$targetExpr), " has no unobserved (stochastic) parents.")
