...
#' 
#' @usage \code{nimEigen(x, only.values = FALSE)}
#'
#' @param x a symmetric numeric matrix (dobule or integer) whose spectral decomposition is to be computed.
#' @param only.values if TRUE, only the eigenvalues are computed, otherwise both eigenvalues and eigenvectors are computed.  (default = \code{TRUE})
#'
...
#' Unlike the \code{eigen} function in base \code{R}, \code{nimEigen} only works for symmetric matrices.  In a \code{nimbleFunction}, \code{eigen} is identical to \code{nimEigen}. 
#' 
#' Additionally, \code{returnType(nimEigen())} can be used within a \code{link{nimbleFunction}} to specify that the function will return a \code{\link{nimbleList}} generated by the \code{nimEigen} function.  \code{nimEigen()} can also be used to define a nested \code{\link{nimbleList}} element.  See the User Manual for usage examples. 
#' 
#'  @value
#'  The spectral decomposition of \code{x} is returned as a \code{\link{nimbleList}} with elements:
#' \itemize{
#' \item values A vector containing the eigenvalues of \code{x}, sorted in decreasing order.  Since \code{x} is required to be symmetric, all eigenvalues will be real numbers.
#' \item vectors. A matrix with columns containing the eigenvectors of \code{x}, or an empty matrix if \code{only.values} is \code{TRUE}.
#' }
#' 
#' @seealso  \code{\link{nimSvd}} for singular value decompositions in \code{nimble}. 
...
#' 
#' @usage \code{nimSvd(x, vectors = 'full')}
#'
#' @param x a symmetric numeric matrix (dobule or integer) whose spectral decomposition is to be computed.
#' @param vectors character that determines whether to calculate left and right singular vectors.  Can take values 'none', 'thin' or 'full'.  Defaults to 'full'.  See details.
#'
#' @author NIMBLE development team
#'
#' @export
...
#' 
#' The \code{vectors} character argument determines whether to compute no left and right singular vectors ('none'), thinned left and right singular vectors ('thin'), or full left and right singular vectors ('full').  For a
...
#' 
#' Setting \code{vectors = 'full'} will compute full matrices for U and V, so that U will be of rank n-by-n, and V will be of rank p-by-p.
#' 
#' In a \code{nimbleFunction}, \code{svd} is identical to \code{nimSvd}. 
#'  
#'  \code{returnType(nimSvd())} can be used within a \code{link{nimbleFunction}} to specify that the function will return a \code{\link{nimbleList}} generated by the \code{nimSvd} function.  \code{nimSvd()} can also be used to define a nested \code{\link{nimbleList}} element.  See the User Manual for usage examples. 
#' 
#'  @value
#'  The singular value decomposition of \code{x} is returned as a \code{\link{nimbleList}} with elements:
#' \itemize{
#' \item d A length m vector containing the singular values of \code{x}, sorted in decreasing order.
#' \item v A matrix with columns containing the left singular vectors of \code{x}, or an empty matrix if \code{vectors = 'none'}.
#' \item u A matrix with columns containing the right singular vectors of \code{x}, or an empty matrix if \code{vectors = 'none'}.
#' }
#' 
#' @seealso  \code{\link{nimEigen}} for spectral decompositions in \code{nimble}. 
