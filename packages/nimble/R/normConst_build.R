...
normImpSamp = nimbleFunction(
  ...
  run = function(vals = double(2), nSamps = integer(0),  nImpSamps = integer(0)){
    ...
    out[1] <- log(mean(normCEst))
    out[2] <- sd(normCEst)  # also return st. dev. of importance weights
    returnType(double(1))
    return(out)
  }
)
...
#' The getSD() method returns the standard deviation of the importance weights used to approximate the normalizing constant.  
#' 
...
#' An idea of the Monte Carlo errror in the approximation can be gotten by the getCV() function, which will return the coefficient of variation (st. dev./mean) of the 
#' importance sampling weights.  A high CV may indicate large Monte Carlo variablility in the approximation.  The getCV() function will return 0 if 
#' it is run before the normalizing constant has been estimated.
#'
...
getNormConst <-nimbleFunction(
  ...
  run = function(mcmcSamps = integer(0), importanceSamps = integer(0)){
    ...
    ## use importance sampling to estimate normalizing constant and CV of importance weights
    impFuncOutput <- impSampFunc$run(vars, numSamps, importanceSamps)
    normConst <- impFuncOutput[1]
    CV <<- impFuncOutput[2]/exp(normConst)
  ...
  ## A method to return the CV  of the importance sampling weights.
  ## A high CV value may indicate that more mcmc or importance weights should be used, 
