...
## Build Laplace
laplace <- buildLaplace(pump, paramNodes, randomEffectsNodes, calcNodes, 
                        control = list(split = FALSE))
claplace <- compileNimble(laplace, project = pump)
claplace$set_method(2)
p <- values(pump, paramNodes)
nimtime <- system.time(nimres <- claplace$LaplaceMLE(p))
## Close to results of TMB
source("pumpTMB.R")
rbind(nimres$par, tmbres$par)
...
llFun_Laplace <- nimbleFunction(
  setup = function(model, paramNodes, randomEffectsNodes, calcNodes, otherNodes) {
    laplace <- buildLaplace(model, paramNodes, randomEffectsNodes, calcNodes, 
                            control = list(split = FALSE, warning = FALSE))
  },
  run = function() {
