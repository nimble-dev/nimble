...
#'
#' @param name Character name of optimization method that can be referred to by
#'   the \code{method} argument of \code{nimOptim} (aka \code{optim} in a
#'   nimbleFunction).
#'
#' @param value An optimization function with specifications described below and
#'   in \link{\code{nimOptim}}. If \code{value} is \code{NULL}, then \code{name}
#'   will be cleared from NIMBLE's set of registered optimizer names. If
...
#' automatically to \link{\code{nimOptim}}, provides a generalization of R's
...
#' which may be NULL).
#'
#' If the call to \code{nimOptim} had \code{hessian=TRUE}, that will be passed
...
#' \code{fn} or \code{he} as functions that will call the compiled (by nimble)
