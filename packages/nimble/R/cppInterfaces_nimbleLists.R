...
#' 
#' 
buildNimbleListInterface <- function(refName,  compiledNodeList, basePtrCall, where = globalenv()){
  defaults <- list()
  if(inherits(compiledNodeList, 'symbolTable')) {
    symTab <- compiledNodeList
    defaults$cnf <- NULL
    warning('No compiled node list provided, so interface will be incomplete')
  } else {
    symTab <- compiledNodeList$nimCompProc$setupSymTab
    defaults$cnf <- compiledNodeList
  }
  ## The following is really equivalent, because it comes *directly* from the place that generates the C++ code
  cppNames <- compiledNodeList$objectDefs$getSymbolNames()
  ...
  fun <- substitute(function(nfObject, defaults, dll = NULL, project = NULL, ...){		#cModel removed from here
    defaults$cnf$nimCompProc$evalNewSetupLinesOneInstance(nfObject, check = TRUE) ## in case this instance was not used during nfProc$process
