...
#' \itemize{
#' \item simNodes. The \code{simNodes} argument specifies the set of model nodes which are simulated, prior to the second calculation of the discrepancy measure.  \code{simNodes} may include any model nodes, including model parameters, latent states, posterior predictive quantities, and/or data values.  Data values specified in \code{simNodes} will indeed be simulated over, but the original values of all \code{simNodes} are restored into the model object, before the MCMC proceeds onward.  If omitted, then \code{simNodes} is taken as an empty vector of node names, and thus no nodes will be simulated.
