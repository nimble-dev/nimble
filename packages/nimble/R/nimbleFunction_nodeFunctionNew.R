    ...
    if(type == 'determ') {
        methodList <- eval(substitute(
            list(
                ...
                calculate  = function(INDEXEDNODEINFO_ = internalType(indexedNodeInfoClass)) { browser(); simulate(INDEXEDNODEINFO_ = INDEXEDNODEINFO_);    returnType(double());   return(invisible(0)) },
