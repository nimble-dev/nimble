...
CRP_conjugate_dnorm_dnorm <- nimbleFunction(
  ...
  methods = list(
    ...
    sample = function(i = integer(), j = integer()) {
      ...
      model[[marginalizedVar]][j] <<- rnorm(1, postMean, sqrt(postVar)) # model[[marginalizedNodes[i]]] <<- rnorm(1, postMean, sqrt(postVar)) can not be accesed  when compiling the MCMC configuration
    }
  )
)
