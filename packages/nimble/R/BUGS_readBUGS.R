...
BUGSmodel <- function(code, name, constants=list(), dimensions=list(), data=list(), inits=list(), returnModel=FALSE, where=globalenv(), debug=FALSE, check=getNimbleOption('checkModel'), calculate = TRUE, envir = parent.frame()) {
    ...
    md$setupModel(code=code, constants=constants, dimensions=dimensions, envir = envir, debug=debug)
...
#' @param envir environment in which if-then-else statements in BUGS code will be evaluated; intended primarily for internal use only
...
nimbleModel <- function(code, constants=list(), data=list(), inits=list(), dimensions=list(), returnDef = FALSE, where=globalenv(), debug=FALSE, check=getNimbleOption('checkModel'), calculate = TRUE, name, envir = parent.frame())
    BUGSmodel(code, name, constants, dimensions, data, inits, returnModel = !returnDef, where, debug, check, calculate, envir)
