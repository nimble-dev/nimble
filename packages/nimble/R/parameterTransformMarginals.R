...
#' }
#'
#' @see also \code{\link{buildLaplace}}
#'
#' @author Daniel Turek & edited by Paul van Dam-Bates for ApproxPosteriors
#' 
#' @export
parameterTransformI <- nimbleFunction(
    ...
    setup = function(model, nodes, control = list()) {
        ...
        tLength <- transformData[nNodes,TIND2]
				...
				## Add some constants to differentiate groups for theta -> p transforms.
				nodeNames <- model$expandNodeNames(nodesExpanded, returnScalarComponents = TRUE)
				countPNodes <- diff(c(0, transformData[,NIND2]))
				countTNodes <- diff(c(0, transformData[,TIND2]))
				pNodes <- rep(1:nNodes, times = countPNodes)	
				thetaNodes <- rep(1:nNodes, times = countTNodes)
				thetaIndices <- NULL
				tmpj <- 1
				for( i in 1:nNodes ) {
        ...
        if(nLength != length(nodeNames)) stop('something wrong with nLength')
    },
    ...
    methods = list(
        ...
        getTransformedNodes = function() { returnType(double(1)); return(thetaNodes) },
        getParameterNodes = function() { returnType(double(1)); return(pNodes) },
        getTransformedNames = function() { returnType(character(1)); return(thetaNames) },
        getParameterNames = function() { returnType(character(1)); return(nodeNames) },

        transform = function(nodeValuesFromModel = double(1), nodeIndex = integer(0, default = 0)) {
        ...
        inverseTransform = function(transformedValues = double(1), nodeIndex = integer(0, default = 0)) {
        ...
        logDetJacobian = function(transformedValues = double(1), nodeIndex = integer(0, default = 0)) {
