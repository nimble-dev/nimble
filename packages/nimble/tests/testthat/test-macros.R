...
test_that('Macro expansion 1',
{
    ...
    expect_identical(
        ...
        modelInfo = list()),
        list(code=quote(x[1] ~ dnorm(y[1], 1)), modelInfo=list())
    )
...
test_that('Macro expansion 2',
{
    ...
    expect_identical(
        nimble:::codeProcessModelMacros(
            quote(x[1] ~ testMacro(y[1])),
        modelInfo = list()),
        list(code=quote(x[1] ~ dnorm(step(y[1]), 1)), modelInfo=list())
    )
...
test_that('Macro expansion 3',
{
    ...
    expect_identical(
        ...
        list(code=quote(x[1] ~ dnorm(z[5], 1)), modelInfo=list())
    )
...
test_that('Macro expansion 5',
{
    ...
    expect_identical(
        ...
        list(code=quote(x[1] <- y[2] + z[i, j]), modelInfo=list())
    )
...
test_that("checkMacroPars removes intermediate parameters and renames parameter list", {
  ...
  pars <- list(testMacro = c("alpha", "beta", "INT"),
               testMacro = c("gamma", "eps"))

  out <- nimble:::checkMacroPars(pars, old, new)

  expect_equal(names(out), c("testMacro__1", "testMacro__2"))
  expect_equal(out$testMacro__1, c("alpha", "beta"))

  old <- quote(x ~ testMacro())
  new <- quote(x ~ dnorm(alpha, beta))

  pars <- list(testMacro = c("alpha", "beta"))
