...
############################
## distn_tests knownFailures
############################

## dexp is not processed by eigenize_recyclingRuleFunction()
distn_tests[["exp_R.dexp x = double(0) rate = double(1, 3)"]]$knownFailures <- list(
  compilation = TRUE
)
...
modify_on_match(
  distn_tests,
  'dt_nonstandard (?!x = double\\(0\\) df = double\\(0\\)  mu = double\\(0\\) sigma = double\\(0\\))',
  'knownFailures',
  list(
    compilation = TRUE
  ),
...
modify_on_match(
  distn_tests,
  'dunif x = double\\(1, 5\\) min = double\\(1, 3\\) max = double\\(1, 7\\)',
  'knownFailures',
  list(
    method1 = list(
...
#####################################
## distn_with_log_tests knownFailures
#####################################

## unfortunately have to repeat all the knownFailures from distn_tests
## TODO: improve the knownFailure system

distn_with_log_tests[["exp_R.dexp x = double(0) rate = double(1, 3)"]]$knownFailures <- list(
  compilation = TRUE
)
