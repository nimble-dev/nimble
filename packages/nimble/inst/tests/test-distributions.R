...
test_that("dnorm_invgamma density calculations", {
          ## check usage from R
          set.seed(1)
          out <- rnorm_invgamma(1, 0, 1, 1, 1)
          dens <- dnorm_invgamma(out, 0, 1, 1, 1, log = TRUE)
          ...
          m$calculate()
          cm$calculate()
          expect_equal(as.numeric(m$y), cm$y, info = "dnorm_invgamma simulate doesn't match compiled/uncompiled")
          ...
          out <- sapply(seq_len(100000), function(i) rnorm_invgamma(1, 2, 2, 3, 0.5))
          loc_quantiles <- qt_nonstandard(qs, df = 2*3, mu = 2, sigma = sqrt((0.5 / 3) / 2))
          scale_quantiles <- qinvgamma(qs, 3, 0.5)
          loc_smp_quantiles <- quantile(out[1, ], qs)
          scale_smp_quantiles <- quantile(out[2, ], qs)
          names(loc_smp_quantiles) <- NULL
          names(scale_smp_quantiles) <- NULL

          expect_equal(loc_smp_quantiles, loc_quantiles, tol = 0.003, info = 'marginal for location not correct')
          expect_equal(scale_smp_quantiles, scale_quantiles, tol = 0.003, info = 'marginal for scale not correct')

          ## check parameter boundaries
          out <- rnorm_invgamma(1, 0, 0, 2, 20)
          expect_equal(out[1], NaN, info = "zero variance simulate corner case")
          dens <- dnorm_invgamma(out, 0, 0, 2, 20)  
          expect_equal(dens, NaN, info = "zero variance density corner case")
          
          out <- rnorm_invgamma(1, 0, Inf, 2, 20)
          expect_equal(out[1], 0, info = "inf variance simulate corner case")
          dens <- dnorm_invgamma(out, 0, Inf, 2, 20)  
          expect_equal(dens, 0, info = "inf variance density corner case")

          out <- rnorm_invgamma(1, 0, 1, 0, 1)
          expect_equal(out, c(NaN, Inf), info = "zero shape simulate corner case")
          dens <- dnorm_invgamma(out, 0, 1, 0, 1)
          expect_equal(dens, NaN, info = "zero shape density corner case")

          out <- rnorm_invgamma(1, 0, 1, 1, 0)
          expect_equal(out, c(0, 0), info = "zero scale simulate corner case")         
          dens <- dnorm_invgamma(out, 0, 1, 1, 0)
          expect_equal(dens, NaN, info = "zero scale density corner case")

          out <- rnorm_invgamma(1, 0, 1, 0, 0)
          expect_equal(out, c(NaN, Inf), info = "zero shape and scale simulate corner case")
          dens <- dnorm_invgamma(out, 0, 1, 0, 0)
          expect_equal(dens, NaN, info = "zero scale and shape density corner case")
})
