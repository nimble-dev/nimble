...
test_that("test of preventing overwriting of data values by inits:", {
    ...
    xVal <- c(3, NA)
    xInit <- c(4, 4)
    expect_warning(m <- nimbleModel(code, constants = list(x = xVal), inits = list(x = xInit)), "Ignoring non-NA values in inits")
    ...
    expect_warning(m <- nimbleModel(code, data = list(x = xVal), inits = list(x = xInit)), "Ignoring non-NA values in inits")
