...
test_ADModelCalculate(model, name = 'state space model', order = c(0,1))
## order=2 too slow for this model and even order=1 takes a long time
...
## ERROR: can't compile the model when have a dflat
...
## Get Hessian relative error of ~ 0.01.
...
## fails as dconstraint has `if` in its density calculation
...
inits <- list(S = diag(rep(1, 3)), z = rep(0, 4), pr = diag(rep(1,5)))
model <- nimbleModel(code, inits = inits)
model$simulate()
model$calculate()
model$setData('y','w')
test_ADModelCalculate(model, name = 'complicated indexing')
## large gradient/hessian differences
...
## restrict to gradient for certain models
