...
nimSmartPtr<NIMBLE_ADCLASS> NIM_DERIVS_CALCULATE(
  ...
  nimSmartPtr<NIMBLE_ADCLASS> ansList = new NIMBLE_ADCLASS; // This will be returned from this funciton.
  nimSmartPtr<NIMBLE_ADCLASS> thisDerivList;  // Used to store derivative output from individual nodes.
  ...
  for (int i = 0; i < derivOrders.dimSize(0); i++) {
    ...
    if (derivOrders[i] == 2) {
      hessianFlag = true;
      jacobianFlag = true; // If second order is asked for, first order must be calculated as well (although we could technically skip copying this into output)
    }
  }
  ...
  if (valueFlag && (!jacobianFlag && !hessianFlag)) { // If only function value is asked for, skip derivative calculation
  ...
  int thisIndex;
  int thisRowLength;
  vector<MatrixXd> parentJacobians;  // For each node, parentJacobians is repopulated to store the Jacobian matrices of all of the parent nodes of that node.
									 
  vector<vector<vector<VectorXd> > > parentHessians;  // Similar to parentJacobians.  The parent Hessians are
                                                      // actually only used as vectors in our
                                                      // implementation of Faa Di Bruno's theorem, so
													  // stored as such. E.g. parentHessians[i][j][k] would be the
													  // i'th parent node's Hessian array's third dimension (a vector).
													 
  vector<MatrixXd> chainRuleJacobians(length(nodes.parentIndicesList)); // Similar storage as parentJacobians above.  chainRuleJacobians[i] is the Jacobian matrix
																		  // for the i'th node (as determined by nodes.parentIndicesList). Values of chainRuleJacobians
																		  // are not overwritten, but stored for use in populating the parentJacobians of downstream nodes.
  vector<vector<MatrixXd> > chainRuleHessians(  // Due to the linear algebra necessary for chain rule calculations, 
	  length(nodes.parentIndicesList));         // the chainRuleHessians are stored in a different format than the parent Hessians.  
                                         // The outer vector goes over
                                         // nodes.parentIndicesList. The second vector goes
                                         // over the third dimension of the Hessian (the third dimension has length = sum length of wrt arguments).
                                         // Then individual MatrixXds are first
                                         // and second dims of the Hessian.

  for (int i = 0; i < length(nodes.parentIndicesList); i++) {
    isDeterminisitic = 1 - nodes.stochNodeIndicators[i]; // Is node i deterministic?
    isWrtLine = (nodes.cumulativeWrtLineNums[i] >= 0);  // Is node i a wrt node (i.e. was it included in the wrt argument to nimDerivs?)
    isCalcNodeLine = nodes.calcNodeIndicators[i]; // Is node i a node included in the call to calculate(nodes) ?
    thisWrtLine = nodes.cumulativeWrtLineNums[i]; // If node i is a wrt node, which wrt node is it (i.e. where does it fall in the topological ordering of wrt nodes) ?
    thisNodeSize = nodes.nodeLengths[i]; // Total size (= product of lengths of dimensions) of this node
    if (isCalcNodeLine) { 

	  // If this node was included in the call to calculate(nodes), we first populate its parent derivative information.  

      vector<int> thisWrtNodes(length(nodes.parentIndicesList[i])); // for each parent node,  do we need to take derivatives wrt that node?
      vector<int> thisHessianNodes(length(nodes.parentIndicesList[i])); // for each parent node,  do we need to calculate hessians wrt that node?
      parentJacobians.resize(length(nodes.parentIndicesList[i]));
      if (hessianFlag) {
        parentHessians.resize(length(nodes.parentIndicesList[i]));
      }
      for (int j = 0; j < length(nodes.parentIndicesList[i]); j++) { // Iterate over all arguments to calcWithArgs() for node i and populate parent deriv info.
        // we can pre-calculate sumParentDims earlier.  Do this.
		if (j == 0 && isWrtLine) {  // This is the case where this node (node i) was included in the wrt argument to nimDerivs.  Note that for the purposes of the chain rule,
			                        // we set the parent 1st derivatives of this node wrt itself (its first argument) as a matrix with 1s on the diagonal. 
          thisWrtNodes[j] = 1;
		...
		} else if (nodes.parentIndicesList[i][j][0] > -1) {  // This is the case where at least one of the parent nodes of the j'th argument to this node (node i)'s 
															 // calculateWithArgs() function depends on a wrt node.  
															 // We populate parentJacobians and parentHessians by retreiving the parent nodes' already calculated derivatives
															 // from the chainRuleJacobians and chainRuleHessians.  In  the case where the j'th argument depends on multiple nodes, we piece
															 // together the parentJacobians and parentHessians from multiple entires in chainRuleJacobians and chainRuleHessians.
          ...
          int sumRowLengths = 0;
		  for (int k = 0; k < nodes.parentIndicesList[i][j].dimSize(0); k++) { // Populating Jacobians and Hessians for parents is somewhat complicated by the fact that
																			   //  argument j can come from multiple different nodes.  I.e., in BUGS code:
																			   //  x[1] ~ dnorm(...)
																			   //  x[2] ~ dnorm(...)
																			   //  y[1:2] ~ dmnorm(x[1:2], ...)
																			   // Here the second argument to y[1:2]'s calculateWithArgs function would be x[1:2],
																			   // and the parent derivative information would have to come from both x[1] and x[2].
																			   //  The derivative information from x[1] and x[2] are combined into a single (matrix / array)
																			   //  to enable the linear algebra necessary to calculate the (Jacobian / Hessian).
            thisRowLength =
                chainRuleJacobians[nodes.parentIndicesList[i][j][k]].rows();
        ...
        } else {  // Otherwise, the j'th argument has no parents that depend on wrt nodes.
          thisWrtNodes[j] = 0;
        }
      }
	  if (nodes.cppWrtArgIndices[i][0] > -1) { // -1 is used as an indicator that node i doesn't have any parents that depend on WRT arguments, so its derivs
	                                           // don't need too be taken.  Otherwise, we proceed with the chain rule.
        thisDerivList =
            instructions[i].nodeFunPtr->calculateWithArgs_derivBlock(
                instructions[i].operand, derivOrders,
                nodes.cppWrtArgIndices[i]); //Derivatives of calculate() for node i are computed here.
        vector<Map<MatrixXd, Unaligned, EigStrDyn> > thisHessian;
		derivOutputFlag = (isDeterminisitic) ? false : true; // derivOutputFlag is true if the derivative output from this node will be added to the ansList.
		if (derivOutputFlag) {  // If derivOutputFlag == true, these derivatives will be taken of a function that returns a scalar (a density calculation),
			                    // so the storage is sized appropriately.
        ...
        } else {// If derivOutputFlag == false, these derivatives will be taken of a deterministic function that returns an object of thisNodeSize,
			    // so the storage is sized appropriately. 
        ...
        for (int j = 0; j < nodes.wrtLineNums.dimSize(0); j++) { // Next we iterate over all the wrt nodes.  For each wrt node,
                                                                 // we iterate over all of the nodes that are arguments to this node's
                                                                 // calculateWithArgs() function.  If argument k depends on wrt node j,
                                                                 // we calcuate the first and second derivatives (stored in chainRuleJacobians[i] and chainRuleHessians[i]) 
                                                                 // of this node (node i) wrt wrt node j using Faa
                                                                 // di Bruno's formula.  
          int thisArgIndex = 0;
          ...
          if (derivOutputFlag) {  // Add this Jacobian to the output of this function
          ...
          if (hessianFlag) {
            for (int j2 = j; j2 < nodes.wrtLineNums.dimSize(0); j2++) {
              ...
              if (derivOutputFlag) { // Add this Hessian to the output of this function
      ...
      } else { // Otherwise, no arguments depend on wrt nodes, so we don't need to take derivatives.  
               // Instead, just set Jacobian and Hessian to 0.
    ...
    if (!isDeterminisitic) { // If this is a wrt node, we need to set the chainRule derivatives appropriately so that the chain rule
