...
void OptimControlNimbleList::copyFromSEXP(SEXP S_nimList_) {
  ...
  SET_STRING_ELT(S_pxData, 0, mkChar(".xData"));
  PROTECT(S_trace =
              findVarInFrame(GET_SLOT(S_nimList_, S_pxData), install("trace")));
  ...
  PROTECT(S_parscale = findVarInFrame(GET_SLOT(S_nimList_, S_pxData),
                                      install("parscale")));
  PROTECT(S_ndeps =
              findVarInFrame(GET_SLOT(S_nimList_, S_pxData), install("ndeps")));
  PROTECT(S_maxit =
              findVarInFrame(GET_SLOT(S_nimList_, S_pxData), install("maxit")));
  PROTECT(S_abstol = findVarInFrame(GET_SLOT(S_nimList_, S_pxData),
                                    install("abstol")));
  PROTECT(S_reltol = findVarInFrame(GET_SLOT(S_nimList_, S_pxData),
                                    install("reltol")));
  PROTECT(S_alpha =
              findVarInFrame(GET_SLOT(S_nimList_, S_pxData), install("alpha")));
  PROTECT(S_beta =
              findVarInFrame(GET_SLOT(S_nimList_, S_pxData), install("beta")));
  PROTECT(S_gamma =
              findVarInFrame(GET_SLOT(S_nimList_, S_pxData), install("gamma")));
  PROTECT(S_REPORT = findVarInFrame(GET_SLOT(S_nimList_, S_pxData),
                                    install("REPORT")));
  PROTECT(S_type =
              findVarInFrame(GET_SLOT(S_nimList_, S_pxData), install("type")));
  PROTECT(S_lmm =
              findVarInFrame(GET_SLOT(S_nimList_, S_pxData), install("lmm")));
  PROTECT(S_factr =
              findVarInFrame(GET_SLOT(S_nimList_, S_pxData), install("factr")));
  PROTECT(S_pgtol =
              findVarInFrame(GET_SLOT(S_nimList_, S_pxData), install("pgtol")));
  PROTECT(S_temp =
              findVarInFrame(GET_SLOT(S_nimList_, S_pxData), install("temp")));
  PROTECT(S_tmax =
              findVarInFrame(GET_SLOT(S_nimList_, S_pxData), install("tmax")));
...
SEXP OptimControlNimbleList::copyToSEXP() {
  ...
  if (!RCopiedFlag) {
    ...
    defineVar(install("trace"), S_trace, GET_SLOT(RObjectPointer, S_pxData));
    defineVar(install("fnscale"), S_fnscale,
              GET_SLOT(RObjectPointer, S_pxData));
    defineVar(install("parscale"), S_parscale,
              GET_SLOT(RObjectPointer, S_pxData));
    defineVar(install("ndeps"), S_ndeps, GET_SLOT(RObjectPointer, S_pxData));
    defineVar(install("maxit"), S_maxit, GET_SLOT(RObjectPointer, S_pxData));
    defineVar(install("abstol"), S_abstol, GET_SLOT(RObjectPointer, S_pxData));
    defineVar(install("reltol"), S_reltol, GET_SLOT(RObjectPointer, S_pxData));
    defineVar(install("alpha"), S_alpha, GET_SLOT(RObjectPointer, S_pxData));
    defineVar(install("beta"), S_beta, GET_SLOT(RObjectPointer, S_pxData));
    defineVar(install("gamma"), S_gamma, GET_SLOT(RObjectPointer, S_pxData));
    defineVar(install("REPORT"), S_REPORT, GET_SLOT(RObjectPointer, S_pxData));
    ...
    defineVar(install("factr"), S_factr, GET_SLOT(RObjectPointer, S_pxData));
    defineVar(install("pgtol"), S_pgtol, GET_SLOT(RObjectPointer, S_pxData));
...
void NIMBLE_ADCLASS::copyFromSEXP(SEXP S_nimList_) {
  ...
  PROTECT(S_value =
              findVarInFrame(GET_SLOT(S_nimList_, S_pxData), install("value")));
  PROTECT(S_gradient = findVarInFrame(GET_SLOT(S_nimList_, S_pxData),
                                      install("gradient")));
  PROTECT(S_hessian = findVarInFrame(GET_SLOT(S_nimList_, S_pxData),
                                     install("hessian")));
  PROTECT(S_thirdDerivs = findVarInFrame(GET_SLOT(S_nimList_, S_pxData),
                                         install("thirdDerivs")));
...
SEXP NIMBLE_ADCLASS::copyToSEXP() {
  ...
  if (!RCopiedFlag) {
    ...
    defineVar(install("value"), S_value, GET_SLOT(RObjectPointer, S_pxData));
    defineVar(install("gradient"), S_gradient,
              GET_SLOT(RObjectPointer, S_pxData));
    defineVar(install("hessian"), S_hessian,
              GET_SLOT(RObjectPointer, S_pxData));
    defineVar(install("thirdDerivs"), S_thirdDerivs,
              GET_SLOT(RObjectPointer, S_pxData));
    RCopiedFlag = true;
    UNPROTECT(5);
  }
