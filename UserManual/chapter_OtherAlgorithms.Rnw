...
\end{enumerate}
\item The \cd{nimbleFunction} provided to \cd{pfType} must have a \code{run} function that accepts a single integer arugment (the number of particles to use), and returns a scalar double (the log-likelihood estimate).
\end{enumerate}
