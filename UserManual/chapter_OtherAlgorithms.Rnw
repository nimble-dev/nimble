...
The EXPERIMENTAL \cd{runCrossValidate} function in NIMBLE performs k-fold cross-validation on a \cd{nimbleModel}.  More information can be found by calling \cd{help(runCrossValidate)}.
