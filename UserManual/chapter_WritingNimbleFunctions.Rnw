...
my_RW <- nimbleFunction(
    ...
    setup = function(model, mvSaved, target, control) {
        scale <- control$scale   ## proposal standard deviation
...
\begin{itemize}
  \item The nimbleFunction must include the contains statement \cd{contains = sampler\_BASE}.
    \item The \cd{setup} function must have the exact four arguments
...
example, the proposal standard deviation \cd{scale}.  Second, to
