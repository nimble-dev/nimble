...
\chapter{Bayesian non parametric models}
\label{cha:bnp}


\section{Bayesian non parametric mixture models}
\label{sec:bnpmixtures}

NIMBLE provides support  for Bayesian non-parametric (BNP) mixture modeling.  The current implementation provides support for hierarchical specifications involving Dirichlet process (DP) mixtures \citep{ferguson;73,ferguson;74,lo;84,escobar;94, escobar;west;95}.    More specifically, a DP mixture model takes the form

$$y_i \mid G \overset{iid}{\sim} \int h(y_i \mid \theta) G(d\theta), (1)$$
$$G \mid \alpha, G_0 &\sim  DP(\alpha, G_0),$$

\noindent where $h(\cdot \mid \theta)$ is a suitable kernel with parameter $\theta$,  and  $\alpha$  and $G_0$ are the concentration and  baseline distribution parameters of the DP, respectively. DP mixture models can be written with different levels of hierarchy, all being equivalent to  model (1) (I would like to add this a a reference to (1)).
...
$$\boldsymbol{z}\mid \alpha \sim CRP(\alpha),\hspace{0.5cm}  \tilde{\theta}_j \overset{iid}{\sim}G_0,$$
...
$$y_i \mid {\boldsymbol{\theta}}^{\star}, \boldsymbol{v} \overset{ind}{\sim} \sum_{l=1}^{\infty}\left\{ v_l\prod_{m<l}(1-v_m)\right\} h(\cdot \mid {\theta}_l^{\star}), (3) $$
$$v_l \mid \alpha \overset{iid}{\sim} Beta(1, \alpha),\hspace{0.5cm}  {\theta}_l^{\star} \overset{iid}{\sim}G_0.$$

\noindent A finite dimensional approximation can be obtained by truncating the infinite sum to have $L$ components.
...
The \cd{conc} parameter is the concentration parameter of the CRP,  controlling the probability of a customer sitting on a new table, i.e., creating a new cluster. The \cd{size} parameter defines the size of the integers set to be partitioned. See \cd{help('dCRP')} for details of these parameters. 

The \cd{conc} parameter is  a positive real value that can be treated as known or unknown. When a gamma prior with fixed parameters is assumed for the \cd{conc} parameter, a specialized sampler is assigned. See more on this in section \ref{sec:mcmcdcrp}.
...
$$p(z_i  = m \mid z_{-i}, \alpha) = \frac{1}{n-1+\alpha} \sum_{j \neq i}\indicator_{\{z_j\}} (m)+ \frac{\alpha}{n-1+\alpha}\indicator_{\{z^{new}\}}(m),$$

where $z_{-i}$ denotes vector $\boldsymbol{z}$ after removing its $i-$th component, $z^{new}$ is a value not in $z_{-i}$, and  $\indicator_{A}$ denotes the indicator function at set $A$.
...
The following example illustrates how to use NIMBLE to perform single density estimation for real-valued data, under a BNP approach, using the \cd{dCRP} distribution. The model is given by  
...
$$ \boldsymbol{z} \sim CRP(\alpha), \hspace{0.5cm} \alpha \sim Gamma(1, 1), $$
$$ \tilde{\theta}_j \overset{iid}{\sim} N(0, 100),  \hspace{0.5cm}\tilde{\sigma}^2_j \overset{iid}{\sim}InvGamma(1, 1),  \hspace{0.2cm} j=1, \ldots, M. $$
...
NIMBLE parametrizes the \cd{stick\_breaking} function by a $(0,1)-$valued vector.

\subsubsection*{Specification}

The \cd{stick\_breaking} function is specified in NIMBLE for a probabilities vector \cd{w} as 
\texttt{w[1:L] <- stick\_breaking(v[1:(L-1)])}

Parameter \cd{v}  is a vector of values between 0 and 1 defining  the breaking length of the stick. It is of length $L-1$, implicitly assuming that its last component is equal to 1.

In order to complete the definition of the weights in the stick-breaking representation of $G$, a beta prior distribution has to be assumed for $v_l$, $l =1, \ldots, L-1$.\\
...
$$ \boldsymbol{z} \sim Discrete(\boldsymbol{w}), \hspace{0.5cm} v_l\overset{iid}{\sim} Beta(1, 1), \hspace{0.2cm}l=1, \ldots, L-1, $$
$${\theta}^{\star}_l \overset{iid}{\sim} N(0, 100),  \hspace{0.5cm}{{\sigma}^2}^{\star}_l  \overset{iid}{\sim}InvGamma(1, 1),  \hspace{0.2cm} l=1, \ldots, L.$$
...
Note that both samplers are specialized versions that operate on a vector having a CRP distribution. Details of these assignments are strictly internal to the CRP samplers.  Additionally, a specialized sampler is assigned to the \cd{conc} hyper parameter when a gamma hyper prior, with fixed parameters, is assigned, see section 6 in \cite{escobar;west;95} for more details. Otherwise, a random walk Metropolis-Hastings sampler is assigned, case where \cd{NaN} can be produced in the MCMC steps.

\subsubsection{Initial values}

Valid initial values should be provided for all elements of the process specified by a CRP structure before running the MCMC. A simple and safe choice for \cd{z} is to provide a sample of size \cd{N}, the same as its length, of values between 1 and 10, with replacement, as done in the preceding CRP example. For the concentration parameter, a safe initial value is 1.

\subsubsection{Sampling the random measure}

In  BNP models, it is oftenly of interest to make inference about the unknown measure $G$.  NIMBLE provides a sampler for this random measure when a CRP structure is involved in the model. This sampler is implemented  as  \cd{sampler\_DP\_measure}. More details for using this function are available using \cd{help('sampler\_DP\_measure')}.
...
<<DP_measure_Chunk, results = "hide", message = FALSE>>=
...
rdens <- nimble:::sampler_DP_measure(Rmodel, mRmodelMCMC$mvSamples)
...
NIMBLE's MCMC engine provides specialized samplers for the \cd{stick\_breaking} function, updating each component of the weight vector sequentially. The sampler is assigned based on inspection of the model structure. Specifically, the specialized sampler is assigned  when the membership vector has a categorical distribution,  its weights are defined by a stick-breaking function, and the  vector defining the weights follows a beta distribution.

\subsubsection{Initial values}

Valid initial values should be provided for all elements of the stick-breaking function and membership variable  before running the MCMC. A simple and safe choice for $z$ is to provide a sample of size \cd{N},  of values between 1 and 10,  with replacement, $10<L$, as done in the preceding stick-breaking example. For the stick variables, safe initial values can be simulated from a beta distribution.
