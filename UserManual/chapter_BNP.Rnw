...
$$p(z_i  = m \mid z_{-i}, \alpha) = \frac{1}{n-1+\alpha} \sum_{j \neq i}\mathbbm{1}_{\{z_j\}} (m)+ \frac{\alpha}{n-1+\alpha}\mathbbm{1}_{\{z^{new}\}}(m),$$

where $z_{-i}$ denotes vector $\boldsymbol{z}$ after removing its $i-$th component, $z^{new}$ is a value not in $z_{-i}$, and  $\mathbbm{1}_{A}$ denotes the indicator function at set $A$.
