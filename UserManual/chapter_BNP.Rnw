...
$$p(z_i  = m \mid z_{-i}, \alpha) = \frac{1}{n-1+\alpha} \sum_{j \neq i}1_{\{z_j\}} (m)+ \frac{\alpha}{n-1+\alpha}1_{\{z^{\mbox{new}}\}}(m),$$

\noindent where $z_{-i}$ denotes vector $\boldsymbol{z}$ after removing its $i-$th component, $z^{\mbox{new}}$ is a value not in $z_{-i}$, and  $1_{A}$ denotes the indicator function at set $A$.
...
$$ \boldsymbol{z} \sim \mbox{CRP}(\alpha), \hspace{0.5cm} \alpha \sim Gamma(1, 1), $$
