...
%% so perhaps we leave in the remaining material rather than putting in roxygen and simply take out the above argument list and refer to the help page for dcarnormal and then lead into this with a sentence that says that when using dcarnormal in BUGS code, to note the following. 
