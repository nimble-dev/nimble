...
distribution. Addition details of using this distribution are available using \cd{help(CAR-Normal)}.
...
The adjacency structure defined by \cd{adj} must be symmetric.  That is, if region $i$ is neighbor of region $j$, then region $j$ must also be a neighbor of region $i$.  In addition, the normalized weights specified in \cd{C} must satisfy a symmetry constraint jointly with the conditional variances given in \cd{M}.  This constraint requires that $M^{-1}C$ is symmetric, where $M$ is a diagonal matrix of conditional variances and $C$ is the normalized weight matrix.  Equivalently, this implies that $C_{ij}M_{jj} = C_{ji}M_{ii}$ for all pairs of neighboring regions $i$ and $j$.  NIMBLE performs a check of these symmetries and will issue an error message if asymmetry is detected.

Two options are available to simplify the process of constructing the \cd{C} and \cd{M} arguments.  First, these arguments may be omitted from the \cd{dcar\_proper} specification.  In this case, values of \cd{C} and \cd{M} will be generated that correspond to all weights being equal to one, or equivalently, a symmetric weight matrix containing only zeros and ones.  Note that \cd{C} and \cd{M} should either \emph{both} be provided, or \emph{both} be omitted from the specification.

Second, a convenience function \cd{as.carCM} is provided to generate the \cd{C} and \cd{M} arguments corresponding to a specified set of symmetric unnormalized weights.  If \cd{weights} contains a sparse vector representation of the full unnormalized weight matrix (\cd{weights} is precisely the argument which can be used in the \cd{dcar\_normal} specification), then a list containing \cd{C} and \cd{M} can be generated using \cd{as.carCM(adj, weights, num)}.  In this case, \cd{C} is the row-normalized weights, and \cd{M} is a vector of the inverse row-sums of the unnormalized weight matrix.  These two options are both demonstrated in the example.
...
Regions with zero neighbors (defined by a 0 appearing in the \cd{num} parameter) of the \cd{dcar\_normal} distribution are a special case.  The corresponding neighborhood $\mathcal{N}$ of such a region contains no elements, and hence the conditional prior is improper and uninformative, tantamount to a \cd{dflat} prior distribution.  Thus, the conditional posterior distribution of those regions is entirely determined by the dependent nodes, if any.  Sampling of these zero-neighbor regions proceeds as:
