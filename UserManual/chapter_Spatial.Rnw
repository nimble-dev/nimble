...
\item [\cd{zero\_mean}] Integer specifying whether to impose a zero-mean constraint during MCMC sampling.  If equal to \cd{TRUE}, the mean of all CAR process values is enforced to be zero, and a separate intercept term should be included in the model.  Default is \cd{FALSE}.
\end{description}
...
The zero-mean constraint on CAR process values is imposed during MCMC sampling, if the argument \cd{zero\_mean = TRUE}, mimicing the behavior of WinBUGS.  Following the univariate updates on each component, the mean is subtracted away from all process values, resulting in a zero-mean process.
...
To mimic the behavior of WinBUGS, we specify \cd{zero\_mean = TRUE} to enforce a zero-mean constraint on the CAR process, and therefore include a separate intercept term \cd{alpha} in the model. Note that we do not necessarily recommend imposing this constraint, per the discussion earlier in this chapter.
...
code <- nimbleCode({
    ...
    for(k in 1:L)
        weights[k] <- 1
    s[1:N] ~ dcar_normal(adj[1:L], weights[1:L], num[1:N], tau, zero_mean = TRUE)
    for(i in 1:N) {
