...
in \ver), and since you can define model functions and distributions as nimbleFunctions, you can combine these features to build external compiled code or arbitrary R functions into a model.  See (\ref{sec:calling-external-code}-\ref{sec:calling-R-code}).
