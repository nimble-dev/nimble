...
Since NIMBLE allows you to wrap calls to external compiled code or arbitrary R functions as \nm{nimbleFunctions} (experimental features
in \ver), and since you can define model functions and distributions as \nm{nimbleFunctions}, you can combine these features to build external compiled code or arbitrary R functions into a model.  See (\ref{sec:calling-external-code}-\ref{\label{sec:calling-R-code}}).
