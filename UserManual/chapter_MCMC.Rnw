...
The default control parameters governing each of these samplers can be inspected and modified with \cd{nimbleOptions("MCMCcontrolDefaultList")}.  %These default values are described in Section \ref{sec:samplers-provided}, along with the related sampling algorithms.
...
When adding samplers to a configuration, the default control list can be over-ridden.
...
\item The defaults, as defined in \cd{nimbleOptions("MCMCcontrolDefaultList")}.
\end{enumerate}
