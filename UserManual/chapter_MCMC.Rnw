...
\section{One-line invocation of MCMC engine: \cd{nimbleMCMC}}
...
The \cd{nimbleMCMC} function provides control over:
...
This entry point for using \cd{nimbleMCMC} is the \cd{code}, \cd{constants}, \cd{data}, and \cd{inits} arguments which are used for building a NIMBLE model (see Chapters \ref{cha:writing-models} and \ref{cha:building-models}).  However, when using \cd{nimbleMCMC}, the \cd{inits} argument can also specify a list of lists of initial values which will be used for each MCMC chain, or a function which generates a list of initial values, which will be generated at the onset of each chain.  As an alternative entry point, a NIMBLE \cd{model} object can also be supplied to \cd{nimbleMCMC}, in which case this model will be used to build the MCMC algorithm.
...
mcmc.out <- nimbleMCMC(model = Rmodel, nchains = 10, inits = initsFunction,
                       returnSamples = FALSE, returnSummary = TRUE)
...
mcmc.out <- runMCMC(Cmcmc, nchains = 3, returnSummary = TRUE, returnWAIC = TRUE)
