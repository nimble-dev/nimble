...
The \cd{runCrossValidate} function in NIMBLE performs k-fold cross-validation on a \cd{nimbleModel} fit via MCMC.  More information can be found by calling \cd{help(runCrossValidate)}. This feature is EXPERIMENTAL - please report any problems or questions to the NIMBLE development team.
