...
The \cd{run} method has optional arguments \cd{thin} and \cd{thin2}.  These can be used to specify thinning intervals for recording samples (corresponding to \cd{monitors} and \cd{monitors2}).  If either of these arguments are provided, they will override the thinning intervals that were specified in the original MCMC configuration object.
