...
\item Run the MCMC and extract the samples (Sections \ref{sec:runMCMC}, \ref{executing-the-mcmc-algorithm} and \ref{sec:extracting-samples}).
\item Optionally, calculate the WAIC using the posterior samples (Section \ref{sec:WAIC}).
\end{enumerate}
...
\begin{itemize}
\item number of MCMC iterations in each chain;
\item number of MCMC chains to execute;
\item number of burn-in samples to discard from each chain;
\item thinning interval on which samples should be recorded;
\item model variables to monitor and return posterior samples;
...
Several example usages of \cd{nimbleMCMC} are shown below:

<<nimbleMCMC, eval=FALSE>>=
code <- nimbleCode({
    mu ~ dnorm(0, sd = 1000)
    sigma ~ dunif(0, 1000)
    for(i in 1:10)
...
See \cd{help(nimbleMCMC)} for further details.
...
\subsubsection{Printing, removing, and reordering samplers: \cd{printSamplers},\cd{removeSamplers}, \cd{setSamplers}, and \cd{getSamplerDefinition}}

The current, ordered, list of all samplers in the MCMC configuration may be printed by calling the \cd{printSamplers} method. When you want to see only samplers acting on specific model nodes or variables, provide those names as an argument to \cd{printSamplers}.  The \cd{printSamplers} method accepts arguments controlling the level of detail displayed as discussed in its R help information.  A few usage examples are shown below:

<<printSamplers, eval=FALSE>>=
## Print all samplers
...
Samplers may be removed from the configuration object using \cd{removeSamplers}, which accepts a character vector of node or variable names, or a numeric vector of indices.
...
Samplers to retain may be specified reordered using \cd{setSamplers}, which also accepts a character vector of node or variable names, or a numeric vector of indices.
...
Note that if you would like to be able to calculate the WAIC of the model after the MCMC function has been run, you must set \cd{enableWAIC = TRUE} as an argument to either \cd{configureMCMC} or \cd{buildMCMC}, or set \cd{nimbleOptions(enableWAIC = TRUE)}, which will enable WAIC calculations for all subsequently built MCMC functions.  For more information on WAIC calculations, see Section \ref{sec:WAIC} or \cd{help(buildMCMC)} in R.
...
\item Number of burn-in samples to discard from each chain;
\item Thinning interval for recording samples;
\item Initial values, or a function for generating initial values for each chain;
\item Setting the random number seed;
...
% When running a single chain, \cd{runMCMC} returns a matrix of MCMC samples (as described in Section \ref{sec:extracting-samples}).  When running multiple chains, a list of sample matrices is returned.  Using the argument \cd{samplesAsCodaMCMC=TRUE}, a \cd{coda} \cd{mcmc} object, or in the case of multiple chains, a \cd{coda} \cd{mcmc.list} object is returned instead.
...
See \cd{help(runMCMC)} for further details.


\section{Running the MCMC}
\label{executing-the-mcmc-algorithm}

The MCMC algorithm (either the compiled or uncompiled version) can be executed using the member method \cd{mcmc\$run} (see \cd{help(buildMCMC)} in R).  The \cd{run} method has one required argument, \cd{niter}, the number of iterations to be run.

The \cd{run} method has optional arguments \cd{nburnin}, \cd{thin} and \cd{thin2}.  These can be used to specify the number of pre-thinning burnin samples to discard, and the post-burnin thinning intervals for recording samples (corresponding to \cd{monitors} and \cd{monitors2}).  If either \cd{thin} and \cd{thin2} are provided, they will override the thinning intervals that were specified in the original MCMC configuration object.
...
The \cd{runCrossValidate} function in NIMBLE performs k-fold cross-validation on a \cd{nimbleModel} fit via MCMC.  More information can be found by calling \cd{help(runCrossValidate)}. 
