...
At this point, it will be helpful to grasp more of how AD works and its implementation in nimble via the CppAD library (citation).  AD methods work by following a set of calculations multiple times, sometimes in reverse order.  
...
In this example, we will use nimble's Laplace approximation, which uses derivatives internally, to get maximum (approximate) likelihood estimates for the GLMM model above.  Note that the Laplace approximation uses second derivatives, and the gradient of the Laplace approximation uses third derivatves.  These are described in detail by (citation).
