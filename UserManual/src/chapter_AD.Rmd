...
1. how to turn on derivatives in a model and use them for Hamiltonian Monte Carlo (HMC) and Laplace approximation.
...
## How to turn on derivatives in a model 

To allow algorithms to use automatic derivatives for a model, include `buildDerivs=TRUE` in the call to `nimbleModel`. (Note that if you use the one-step method `nimbleHMC` described below, then you do not need to build the model yourself and so you do not need to provide this `buildDerivs` option.)

We'll use a Poisson Generalized Linear Mixed Model (GLMM) as a simple example model for HMC and Laplace approximation. There will be 10 groups (`i`) of 5 observations (`j`) each. Each observation has a covariate, `X`, and each group has a random effect `ran_eff`. Here is the model code:
```{r eval=TRUE}
...
As preparation for the HMC and Laplace examples below, we need to finish setting up the GLMM. We could have provided data in the call to `nimbleModel`, but instead we will simulate it using the model itself. Specifically, we will set parameter values, simulate data values, and then set those as the data use.
...
If you are not very familiar with using a `nimble` model, that might have been
confusing, but it was just for setting up the example.
...
## How to use Hamiltonian Monte Carlo (HMC)

NIMBLE's HMC sampler is provided in a separate package, `nimbleHMC`. HMC is an MCMC method that works by a physical analogy of frictionless motion on a surface that is the negative log posterior density. HMC often achieves good mixing, although at high computational cost. Within `nimble`, HMC is more flexible than in some implementations because HMC can be used on some parts of a model while other samplers are used on (the same or) other parts of the same model. Indeed, @neal-11 discussed using it in this way.

At its core, HMC in NIMBLE is just another MCMC sampler that can be used in NIMBLE's general MCMC system, possibly along with other samplers. For convenience, `nimbleHMC` provides different ways to set up and run HMC that allow different degrees of control.  These include:

- `nimbleHMC` is analogous to `nimbleMCMC`: It takes model `code`, `data`, `inits` and `constants` and does all steps from building the model to running one or more MCMC chains. Use this if you want one call to get default HMC samples of the entire model.
- `configureHMC` is analogous to `configureMCMC`: It sets up a default MCMC configuration that comprises one HMC sampler for all model nodes (or nodes you choose). You can then use `addSampler` and other methods of the configuration to fully customize it before building and using the MCMC.  Use `configureHMC` if you want to do MCMC with both HMC and other samplers and you find it easier to start with HMC in the configuration first.
- `buildHMC` is analogous to `buildMCMC`: It builds a configuration that, if not provided, will be created by `configureHMC`. Use this if you want to manage MCMC runs through `runMCMC` instead of `nimbleHMC` and you don't need to customize the configuration by doing `configureHMC` yourself.
- `addHMC`: This manages use of `addSampler` for an MCMC configuration to add specifically an HMC sampler for a chosen set of nodes. It optionally removes other samplers first. Use this if you want to do MCMC with both HMC and other samplers and you find it easier to start with the other sampler in the configuration, i.e. from the result of `configureMCMC`. 
- For the most fine-grained control, you can call `configureMCMC` and then use the `addSampler` with `type="HMC"` to add HMC sampler(s).

More information on these is available from the help pages, e.g. `help(configureHMC)`.

With the GLMM created above, we will configure and build an MCMC to use HMC. In this case we'll use HMC for the whole model, i.e. all parameters and latent states.
```{r eval=eval}
library(nimbleHMC)
HMC <- buildHMC(model)
CHMC <- compileNimble(HMC, project = model) # The compiled HMC will use the compiled model.
...
Cglmm_laplace <- compileNimble(glmm_laplace, project = model, resetFunctions = TRUE)
```
Note that `resetFunctions=TRUE` is sometimes necessary when compiling a second algorithm to use the same compiled model as a previous algorithm (in this case, the MCMC above, with the HMC sampler).
...
It is possible to activate derivative support in a user-defined function or distribution used in a model. Simply set `buildDerivs=TRUE` in `nimbleFunction`.
...
- values of any arguments that are not of type 'double', including e.g. the 'log' argument in `d_my_norm`. ('d' functions called from models always have `log = TRUE`, so in this case it is not a problem.)
- the evaluation path followed by any if-then-else.
...
Note that for a nimbleFunction without setup code, one can say `buildDerivs=TRUE`, `buildDerivs = 'run'`, or `buildDerivs = list(run = list())`. One can't take derivatives of `nf_sqrt` on its own, but it can be called by a method of a nimbleFunction that can have its derivatives taken (i.e. one with `setup` code). 
...
In general, choice of forward mode versus reverse mode has to do with the lengths of inputs and outputs and possibly specific operations involved. In nimble, some reasonable defaults are used.
...
```
(The "lifted" nodes here are for the `exp(intercept + beta*X[i,j] + ran_eff[i])`, i.e. the inputs to `dpois`. See the NIMBLE User Manual to learn about lifted nodes.)
