...
To change the outer optimizer, one can use the `method` argument to `runLaplace` (and to the `findMLE` method).

To change either the inner optimizer, one can use the `innerOptimMethod` element of the `control` list argument to `buildLaplace`. One can modify various settings that control the behavior of the inner and outer optimizers via `control` as well. See `help(buildLaplace)` for more details.

Once a Laplace approximation is built, one can use `updateSettings` to modify the choice of inner optimizer and various settings that control the behavior of the inner and outer optimizers (see `help(buildLaplace)` for details).
...
```{r eval=FALSE}
## Create an R wrapper function that has the interface needed for NIMBLE and wraps the optimizer of interest.
nimbleTMBnewton <- function(par, fn, gr, he, lower, upper, control, hessian) {
  he_matrix <- function(p) Matrix(he(p), doDiag = FALSE, sparse = TRUE) # Needs to be of class `dsCMatrix`.
  ...
  ans <- list( # What is handled in the return is fairly particular, so often needs conversion from a given method such as TMB::newton.
    par = res$par,
    value = res$value,
    counts = c(0, 0, 0), # Counts of fn, gr, and he calls, but these are not counted by TMB::newton.
    evaluations = res$iterations,
    hessian = NULL, # TMB::newton gives a `dsCMatrix` but we need a base R matrix.
    message = "ran by TMB::newton",
    convergence = 0 # TMB::newton does not return a convergence code so give 0 (i.e., converged).
  )
  return(ans)
}
...
- dynamic indexing that would produce a nonscalar result, such as `p[z[i], 1:3]`. Dynamic indexing that produces a scalar result, such as `p[z[i], j]`, is allowed and can be used as a work-around, e.g., defining `ptmp` as `for(j in 1:3) ptmp[i,j] <- p[z[i],j]` and using it in place of `p[z[i], 1:3]`.

Note that functions and distributions that are not supported can still be used in a model, as long as no algorithm tries to take derivatives with respect to node(s) that is (are) not supported. For example, `x ~ dcat(p[1:3])` declares `x` to be a discrete random variable following a categorical distribution. An algorithm can use derivatives with respect to `p[1:3]` (or with respect to parent nodes of `p[1:3]`) because the calculation of the log probability of `x` given `p[1:3]` is continuous with respect to `p`. However, derivatives with respect to `x` will not work. In the case of truncated distributions, derivatives with respect to neither the parameters nor to the random variable are supported.
...
- Use the `ignore` set or explicit creation of variables as `integer` or `logical` to create regular variable types.
- If you need an argument to have derivative-tracking (as is always the case when called from a model) but derivatives with respect to it don't make sense, use `ADbreak` to assign to a local variable that will "bake in" the value and remove it from derivative tracking. The local variable can be regular (if it is in the `ignore` list or was created first as an `integer` or `logical`) or derivative-tracking.


### Using AD with multiple nimbleFunctions {#sec:AD-multiple-NF}
...
### Understanding more about how AD works: *taping* of operations {#sec:understanding-more-AD}
...
- values of any variables with derivative tracking severed `ADbreak` change. (See \@ref(sec:AD-holding-out).)
...
- In Method 1 above, the NIMBLE compiler automatically determines `constantNodes` and `updateNodes` using `makeModelDerivsInfo` based on the inputs to
`nimDerivs(model$calculate(...),...)`.
...
- If vector `x[1:n] ~ ddirch` in a model, then $g(x[1]) = \logit(x[1])$ and $g(x[i]) = \logit(x[i] / (1-\sum_{k=1}^{i-1}x[i]))$ for $i > 1$. In this case, `x` follows a Dirichlet distribution and thus has the simplex constraint that $\sum_{i=1}^n x[i] = 1$.
