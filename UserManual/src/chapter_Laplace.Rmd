...
To find the posterior mode (maximum a posteriori estimate), one can call `findMAP` instead of `findMLE`.
...
The core functions are `buildNestedApprox` and `runNestedApprox`. These are analogous to the "build" and "run" functions for MCMC and for Laplace.
