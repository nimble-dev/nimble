...
#### Use double for vectors, matrices or arrays that pass from BUGS code to run code
Numeric values within BUGS code are interpreted as type double. This is the case even for stochatic nodes assigned to discrete probability distributions, such as dpois, dmulti, dcat etc. 
As a consequence, care must be taken that vectors, matrices or arrays that are to be passed via BUGS code to run code must be declared as type double and not as type integer.
Doing otherwise will result in compilation failure. 
...
In the `run` function, the `mvSaved` modelValues object is kept up-to-date with
the current state of the model, depending on whether the proposed
changed was accepted.  This is done using the `copy` function, to copy values between the model and `mvSaved` objects.
