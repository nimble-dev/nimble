...
Supported distributions, listed by their 'd' function, include:
  ...
  - `dmnorm_inv_ld(x, mean, mat, prec_param, log)` (AD)
  - `dmnorm_chol(x, mean, cholesky, prec_param, log)` (non-AD)
...
In the last four, `cholesky` stands for Cholesky decomposition of the relevant matrix; 
...
`dmnorm_inv_ld` is for use with AD (the standard `dmnorm_chol` results in very inefficient AD-based calculation). `prec_param` is a logical indicating whether `mat` is the precision matrix (TRUE) or covariance matrix (FALSE).
...
to simply `i`, the compiler does not determine that.  Since there is
