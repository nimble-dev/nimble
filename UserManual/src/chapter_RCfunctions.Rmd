...
In addition to the above functions, we provide functions `any_na()` and `any_nan()` for finding if there are any `NA` or `NaN` values in a vector. These are equivalent to using `any(is.na())` and `any(is.nan())` in R.
...
length (default 0), and the `value` argument specifies the initial 
 ...
 0) or a vector. If a vector and its length is not equal to `length`, the remaining values will be zero, but we plan to implement R-style recycling in the next version of NIMBLE. The `init` argument specifies 
...
Here is a guide to such issues.  Suppose `v1` and `v2` are vectors, and `M1` is a matrix.  Then
  ...
  -  `v1 + M1` generates a compilation error unless one dimension of
