...
plan(multiprocess, workers=8)  # or some other plan
plan(multiprocess, workers=8)  # or some other plan
```
...
Numeric scalar and matrix mathematical operations are listed in Tables \@ref(tab:functions)-\@ref(tab:functionsMatrix).
...
system("cat tables/functionTableMatrixLong.md tables/eigenSvdTableMatrixAsNimList.md > tmp.md")
```
```{r, child = 'tmp.md'}
```
```{r echo=FALSE, include=FALSE}
...
not support methods 'SANN' and 'Brent'. NIMBLE's `nimOptim` supports
...
(via `\dots`), but a work-around is to create a new `nimbleFunction` that
binds those fixed parameters. Finally, `nimOptim` requires a `nimbleList`
...
the `nimOptimDefaultControl` function, and set any desired fields. For
example usage, see the unit tests in
[tests/test-optim.R](https://github.com/nimble-dev/nimble/blob/master/packages/nimble/inst/tests/test-optim.R).
