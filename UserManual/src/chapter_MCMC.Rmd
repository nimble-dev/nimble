...
Note that if you would like to be able to calculate the WAIC of the model, you must set `enableWAIC = TRUE` as an argument to either `configureMCMC` or  `buildMCMC` (when passing a model rather than an MCMC configuration to `buildMCMC`), or set `nimbleOptions(MCMCenableWAIC = TRUE)`, which will enable WAIC calculations for all subsequently built MCMC functions.  For more information on WAIC calculations, see Section \@ref(sec:WAIC) or `help(waic)` in R.
...
The WAIC [@Watanabe_2010] can be calculated from the posterior samples produced during the MCMC algorithm.  Note that in order to calculate the WAIC value, the argument `enableWAIC = TRUE` must be supplied to `configureMCMC` or to `buildMCMC` (if `buildMCMC` is called with a model instead of an MCMC configuration), or the `enableWAIC` NIMBLE option must have been set to `TRUE`.
