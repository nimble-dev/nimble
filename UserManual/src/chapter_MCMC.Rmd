...
The MCMC configuration contains information needed for building an MCMC.  When no customization is needed, one can jump directly to the `buildMCMC` step below.  An MCMC configuration is an object of class `MCMCconf`, which includes:
  ...
  - Accumulator variables, to record the cumulative sums and sum-of-squares during sampling, without storing all MCMC samples.
...
By default, the MCMC configuation contains no accumulator variables.
...
#### Accumulator variables: *setAccumulators*, *resetAccumulators*, and *printAccumulators*

Accumulator variables (for recording the cumulative sums and sums-of-squares) can be set using the `setAccumulators` method.  All accumulators can be turned off using `resetAccumulators`, and the current accumulator variables are printed using `printAccumulators`.

A matrix containing the cumulative sums and sums-of-squares can be accessed from the compiled MCMC object as shown below.  The first matrix row contain cumulative sums, and the second row contains cumulative sums-of-squares.  In addition, the total number of accumulated samples can be accessed using the `getAccumulatorIterations` method of the MCMC object.  Note that values are accumulated regardless of any thinning or burn-in specified.

```{r, eval=FALSE}
mcmcConf <- configureMCMC(Rmodel)
mcmcConf$setAccumulators("x", "y")
Rmcmc <- buildMCMC(mcmcConf)
Cmodel <- compileNimble(Rmodel)
Cmcmc <- compileNimble(Rmcmc, project = Rmodel)
Cmcmc$run(10000)
accumulators <- as.matrix(Cmcmc$mvAccumulators)
accumulatorIterations <- Cmcmc$getAccumulatorIterations()
```
...
<!---  `runMCMC` has one mandatory argument: the (compiled or uncompiled) MCMC algorithm.  Other arguments to `runMCMC` are available to control: -->
  ...
  - Returning a WAIC value calculated using (post-burn-in and thinned) samples from all chains.
...
The `run` method has an optional `reset` argument.  When `reset = TRUE` (the default value), the following occurs prior to running the MCMC:
  ...
  1. The internal modelValues `mvAccumulators` object for accumulator variables is reset to contain all zeros, and the number of accumulated iterations is reset to zero.
...
The WAIC is calculated by calling the member method `mcmc$calculateWAIC` (see `help(buildMCMC)` in R for more details).  The `calculateWAIC` method has one required argument, `nburnin`, the number of initial samples to discard prior to WAIC calculation.  `nburnin` defaults to 0. This burn-in is done in addition to any burn-in specified when running the MCMC, and the burn-in is applied to the samples before any thinning specified when setting up or running the MCMC. 
...
## Reversible Jump MCMC for variable selection {#sec:rjmcmc}
...
A common use case for RJMCMC is for variable selection in regression-style problems, where the dimension of the parameter space varies as variables are included and excluded from the regression specification. Variable selection can be set up in NIMBLE without using RJMCMC by using indicator variables to turn regression parameters off and on (see model code below for an example of how indicator variables can be used). However, with that approach MCMC sampling can be inefficient because a given regression parameter can wander widely in the parameter space when the corresponding variable is not in the model if the parameter has a non-informative prior. RJMCMC sampling results in the MCMC not sampling the regression coefficients for variables for iterations where the variables are not in the model.  
...
NIMBLE implements RJMCMC for variable selection using a univariate normal distribution as proposal density and supports two types of model specifications, depending on whether or not indicator variables are included. A reversible jump sampler can be added to an MCMC configuration object by calling the function `configureRJ`. The `configureRJ` function modifies the MCMC configuration to (1) assign samplers that turn on and off variables in the model and (2) modify the existing samplers for the regression coefficients to use 'toggled' versions that are only used when the corresponding variable is currently in the model. In the case where indicator variables are included in the model, sampling to turn on and off variables is done using `RJ_indicator` samplers and in the case where indicator variables are not included, sampling is done using `RJ` samplers. 

<!--  See `help(configureRJ)` for details. -->

The `RJ` and `RJ_indicator` both use a univariate normal distribution as proposal density, whose mean and scale can be provided in the `control` list argument of `configureRJ` (by default `mean = 0` and `scale = 1`). An optional argument called `fixedValue` can be provided in the non-indicator setting; this indicates the value taken by nodes in `targetNodes` when out of the model (by default 0). 
...
Since there are no indicator variables, the `RJ` sampler is assigned directly to `beta[1]` and `beta[2]` along with the `toggled` sampler, which still uses the default sampler for the node, but only if the corresponding variable is in the model at a given iteration. In addition in this case one can set the coefficient to a value different from $0$ via the `fixedValue` argument in the `control` list. 
