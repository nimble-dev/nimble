...
The following cases are particularly important to consider when initializing:
  - In a model with flat priors, NIMBLE cannot generate initial values from those priors.
...
Many user questions and problems end up being related to missing or bad initial values. Some suggestions for diagnosing initialization problems include:
 ...
 - To debug the specific node(s) whose initial value(s) are causing problems, run `model$calculate(nodes)` on specific `nodes` and look for values of `-Inf`, `NA`, or `NaN`.
 - With a compiled or uncompiled model, you can run the model initialization (but no MCMC iterations) with `mcmc$run(niter = 0)`. Then inspect the model object, e.g., `model$variable`, to see the results of initialization.
