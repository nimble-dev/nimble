...
The above are calculated and returned for each MCMC chain, all using the post-burnin samples. The samples and log-probabilities are thinned and the summary statistics (but not the stand-alone means and variances or the WAIC) use the thinned samples.  Additionally, posterior summary statistics are calculated for all chains combined when multiple chains are run.
...
# specified burn-in.  Return samples, overal log-probability,
...
runMCMC(buildMCMC(conf), niter = 100, thin = 20)
```
...
runMCMC(buildMCMC(conf), niter = 5)
```
