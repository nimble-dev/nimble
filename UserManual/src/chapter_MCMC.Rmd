...
- Dynamic indexing is supported, although of course one cannot have derivatives taken with respect to (and hence not do HMC sampling for) the actual index values, since they are discrete.
...
- One can take derivatives with respect to the parameters/arguments of `dcat`, `dconstraint`, and `dinterval`. Hence HMC sampling is supported for nodes that have `dcat`, `dconstraint`, and `dinterval` dependencies (derivatives of the discrete random variables themselves of course cannot be taken).
- Truncated distributions can be used, but one cannot take derivatives of nodes that have truncated nodes as dependencies and hence HMC sampling **cannot** be performed on nodes that have truncated nodes as dependencies.
