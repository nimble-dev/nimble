...
#### Adding samplers to the configuration: *addDefaultSampler* and *addSampler*

Additional samplers may be added to a configuration using either the `addDefaultSampler` or the `addSampler` methods of the `MCMCconf` object.  The first argument (`target`) to either method specifies the node(s) for which samplers should be added, as a character vector.

The `addDefaultSampler` method will determine and assign NIMBLE's default sampling algorithm for each node specified.  This may include conjugate samplers, multivariate samplers, or otherwise.  `addDefaultSampler` accepts several additional arguments, guiding the determination of the default sampler (for example, `useConjugacy` and `multivariateNodesAsScalars`).  See `help(addDefaultSampler)` for details.

In contrast, the `addSampler` method accepts a `type` argument, which specifies the precise type of sampler to assign.  This argument may be provided as a character string or a nimbleFunction object. Valid character strings are indicated in `help(samplers)` (do not include `"sampler_"`).  Added samplers can be labeled with a `name` argument, which is used in output of `printSamplers`.
...
The hierarchy of precedence for control list elements for samplers is:
  ...
  1. The `control` list argument provided to `addDefaultSampler` or `addSampler`;
...
Samplers added by either `addDefaultSampler` or `addSampler` will be appended to the end of current sampler list.  Adding a sampler for a node will *not* automatically remove any existing samplers on that node. 
