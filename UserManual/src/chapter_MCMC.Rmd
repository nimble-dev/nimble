...
The WAIC is extracted by the member method `mcmc$getWAIC` (see `help(waic)` in R for more details) or is available as the `WAIC` element of the `runMCMC` or `nimbleMCMC` output list.
...
Equations 5, 12, and 13 in [@Gelman_etal_2014] (i.e., using \emph{p}WAIC2).
...
For comparing WAIC between two models, [@Vehtari_etal_2017] discuss using the per-observation (or per-data group, more generally) contributions to the overall WAIC values to get an approximate standard error for the difference in WAIC between the models. These element-wise values can be extracted by passing `returnElements = TRUE` to `getWAIC`.
