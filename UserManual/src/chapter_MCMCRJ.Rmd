...
The `configureRJ` function modifies the model configuration, and assigns a `RJ` sampler directly to the nodes specified in `targetNodes` when `priorProb` is provided. Otherwise a `RJ_indicator` sampler is assigned to nodes in `indicatorNodes`. In addition, this functions removes the default NIMBLE sampler assigned to each node in `targetNodes`, and adds a toggled version of the default sampler that is run only when the node is the model.
 
<!--  See `help(configureRJ)` for details. -->

The `RJ` and `RJ_indicator` both uses a univariate normal distribution as proposal density, whose mean and scale can be provided in the `control` list argument of `configureRJ` (by default `mean = 0` and `scale = 1`). The `positive` flag in the `control` list allow for only positive values of the proposal when set to `TRUE`, using a folded normal distribution. 
...
A `RJ_indicator` sampler is assigned to `z[1]` and `z[2]` in place of the `binary` sampler, while sampler for `beta[1]`and `beta[2]` have been assigned to a `toggled` that still uses the default `conjugate_dnorm_dnorm` only when the corresponding indicator is equal to $1$. The two `RJ_indicator` samplers have different means for the proposal distribution and same scale, as given in the `control` list of `configureRJ`.
...
We consider the same setting, but we write the model without the use of indicator variables. 
...
In this case, since there are no indicator variables, we need to pass to `configureRJ` the prior inclusion probabilities for each node in `targetNodes`, by passing either one common value or a vector of values to `priorProb`. Notice that this case does not allow for a stochastic prior. 
...
The same behavior can be obtained also when using indicator variables, using a different model specification. For example, the model in \@ref(sec:rjmcmc-no-indicator} can be modified as
