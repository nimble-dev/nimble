...
# Sequential Monte Carlo and MCEM {#cha:algos-provided}
...
The IF2 is somewhat different than the other filters in that the goal of the algorithm is to find the maximum likelihood estimates of the model parameters. To do so, the algorithm represents both latent states and parameters using particles that are weighted and resampled during the iterations, where the perturbations to the parameter particles follow a schedule of decreasing magnitude to yield convergence to the MLE.
...
In addition to the estimates, we can extract the values of the log-likelihood, the estimates and the standard deviation of the parameter particles as they evolve over the iterations, so as to assess convergence.
...
cFilter$estimates[95:100,]
cFilter$logLik[90:100]
```
...
The  `type = "RW_PF"` and `type = "RW_PF_block"` samplers
