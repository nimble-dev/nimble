...
INLA adjusts the latent node marginals via a Gaussian copula to correspond with a skew-normal approximation [TODO: check on terminology here]. We have not implemented such an approximation so no adjustment is done.
...
It is also possible to choose which nodes go in which set. One might choose to include fixed effects in the parameter set, akin to how Laplace approximation works. Another situation where one might configure this manually is if random effects in a model are specified in a "non-centered" parameterization such as:
...
The core functions are `buildNestedApprox` and `runNestedApprox`. These are analgous to the "build" and "run" functions for MCMC and for Laplace.
 ...
 - Various additional functions can be applied to the nested approximation object to do further components of the approximation, including:
    - `improveParamMarginal`: use AGHQ to improve marginal parameter inference relative to the default inference based on the asymmetric Gaussian approximation.
    ...
    - `sampleParams`: sample from the asymmetric Gaussian approximation.
...
We'll use the penicillin example from the `faraway` package, which has data on penicillin production as a function of treatment (four levels) and blend (five levels). The treatment is considered as a fixed effect (note the constant, large variance/small precisionwhile the blend/block is considered as a random effect.

[insert model code and model set up]


Given a NIMBLE model, we can build (and compiled) our nested approximation.
...
This is a simple model, so it's not surprising that doesn't take long, but even for large models or a large number of parameters, this step will generally be faster than the steps below because the approximate Gaussian distribution for the parameters can be computed quickly, requiring maximization and computation of the Hessian for the Laplace approximation (that said that could take some time when there is a large number of latent nodes).

Now, let's refine our estimation of the parameters by using AGHQ in place of the approximate Gaussian. That does $d-1$ dimensional AGHQ (simply 1-d AGHQ) for each parameter requested (by default all of them).


There are two quantities that control the accuracy of the AGHQ marginal estimate. The first is the number of grid points in the AGHQ numerical integration (`nQuad`), where $k$ is the number of points in one dimension. More is better for accuracy but the computational cost grows as $(d-1)^k$. The marginal density is estimated at each of a set of evaluation points of values of the parameter (`nMarginalGrid`), using AGHQ for each point, after which a spline is fit to estimate the full marginal density.

[show increase in nGrid]
[show increase in nPoints]

We can estimate other expectations or quantiles than those reported and plot a density estimate:

[show emarginal, qmarginal, plotMarginal]
...
#### Setting latent nodes and parameters
