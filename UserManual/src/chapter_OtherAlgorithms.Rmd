...
NIMBLE version 1.4.0 introduces a nested approximation method that provides approximate posterior inference using methodology similar to the well-known INLA approach [@rue2009approximate, @martins2013bayesian] and to the related methods for extended Gaussian latent models (EGLM) of @stringer2023fast, implemented in the `aghq` R package.

Such nested approximations build on Laplace approximation, which provides an approximate marginal posterior, integrating (marginalizing) over latent nodes. Then instead of maximizing the approximation, one approximates the marginal posterior of the (hyper)parameters on a carefully-chosen set of points. Inference for individual (hyper) parameters is done by numerical approximation or numerical integration. Inference for the latent nodes is done via numerical integration (INLA) or via sampling (INLA and EGLM). 
...
Here we list some of the similarities and differences from INLA and the EGLM approach (`aghq` package):
 ...
 - Like EGLM, we take the latent nodes to be only the parameters in the model, without including the additive predictor values as done in INLA.
...
NIMBLE's nested approximation consists of several pieces, with different options that allow a user to choose how the approximation is done. We briefly describe the pieces here. We refer to the (hyper)parameters simply as "parameters". 
...
In the case that the conditional distributions of the latent nodes factor into univariate conditionally independent sets of nodes (conditional on the parameters), this by default uses the product of univariate Laplace approximations and in NIMBLE can be made more accurate by using AGHQ with more than one quadrature point. 

#### Approximating the marginal parameter distribution on a grid

In order to simulate from the posterior distribution for the latent nodes (or to estimate the marginal likelihood), one needs to evaluate the  joint parameter (approximate) posterior density on a grid of points.
...
NIMBLE also allows users to provide their own grids. 

#### Joint inference for latent nodes

NIMBLE provides the ability to simulate the latent nodes from a mixture of multivariate Gaussian distributions, mixing over the parameter values at the grid points discussed in the previous section. The multivariate Gaussian distribution is based on the Laplace approximation for the latent nodes, which provides the mean and variance conditional on the parameter value at the grid point. The weights in the mixture are based on the Laplace-approximated marginal density (for the parameters and data, jointly), with stratified sampling to reduce variance.

INLA adjusts the latent node marginals via a Gaussian copula to correspond with a skew-normal approximation. We have not implemented such an approximation so not adjustment is done.

#### Univariate inference for parameters

NIMBLE offers two approaches for univariate marginal inference for individual parameters. The first is a computationally-efficient integration-free method that mimics that used by INLA [@martins2013bayesian]. This uses the  joint asymmetric Gaussian as the joint marginal posterior of the parameters and allows one to calculate the density at individual evaluation points for the univariate marginal. The univariate density is calculated on a fine grid of points, from which one can build a spline-based approximation to the marginal density that can be used to compute moments and quantiles for univariate inference.

The second, more accurate approach, is to use $d-1$-dimensional AGHQ to integrate over the Laplace-approximated joint parameter marginal distribution. This can greatly improve accuracy, but can be computationally expensive unless $d$ is quite small (e.g., 2-4), or even if the number of latent node elements is quite large (which makes the Laplace approximation expensive). Because of the expense, we only take this approach if requested by the user and we allow the user to choose the specific parameter(s) for which they want to estimate marginals for using this approach.
...
In NIMBLE's nested approximation, the marginal likelihood is approximated either by the approximation used by INLA based on the approximate Gaussian distribution for the parameters or via AGHQ using the AGHQ grid weights and associated Laplace-approximated density values.


#### Determining latent nodes and parameters

By default, NIMBLE's nested approximation will try to automatically determine the node sets, selecting random effects and regression fixed effects (coefficients) as the latent nodes and other unknown quantities (variance components and parameters of random effects distributions) as "parameters".

Given the computations involved, it is best to have the number of parameters be relatively small, such as fewer than 10.

Note that the treatment of fixed effects differs between the nested approximation and NIMBLE's Laplace approximation. In Laplace approximation, fixed effects are treated as parameters of interest and maximized with respect to, rather than being marginalized over. In the nested approximation, they are marginalized over and inference on them can be done based on simulated values.

It is also possible to choose which nodes go in which set. One might choose to include fixed effects in the parameter set. Another situation where one might configure this manually is if random effects in a model are specified in a "non-centered" parameterization such as:
...
In this parameterization the random effects, $b_i$, do not depend on any parameters because $\mu$ and $\sigma$ are involved directly in the linear predictor, $\eta_i$ (often one would instead have a centered parameterization, $b_i \sim \mathcal{N}(\mu, \sigma)$). Because of this, NIMBLE's nested approximation would by default put the $b_i$ nodes into the parameter set, which in general would result in very slow computation (because there would be a large number of parameters). 

#### Computational bottlenecks

To sample the latent nodes, one needs to find the weights and the parameters of the multivariate normal approximation to the latent nodes at each point in the grid of parameters. For a large number of parameters this can be expensive because Laplace approximation is done at many grid points. For for a large number of latent node elements, this can be expensive because one is using a large multivariate normal. Computing the parameters of this involves optimization over a space whose dimension is the number of latent node elements, as well as computation of the Hessian at the maximum. Furthermore, one needs to simulate from the high-dimensional multivariate normal. 

To approximate the univariate marginals for the parameters via AGHQ, this requires $d-1$ dimensional AGHQ, which can be expensive as the number of quadrature points grows as $(d-1)^k$ where $k$ is the number of Gauss-Hermite quadrature grid points in one dimension. Often this would be chosen to be small, such as 3 or 5, but even the computations increase rapidly in $d$. 

### Example usage

[show moving fixed effects back and forth]