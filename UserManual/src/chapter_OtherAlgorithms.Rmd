...
## Monte Carlo Expectation Maximization (MCEM)
  ...
  @Caffo_etal_2005. `buildMCEM` was re-written for nimble version 1.1.1 and is
...
The first argument `buildMCEM`, `model`, is a NIMBLE model. At the moment, the
...
Direct maximization after analytically integrating over the latent nodes
(possible for this model but often not feasible) gives estimates of
$\hat\alpha=0.823$ and $\hat\beta = 1.261$, so the MCEM seems to do pretty well,
though tightening the convergence criteria may be warranted in actual usage.
