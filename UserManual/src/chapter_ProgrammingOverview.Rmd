...
This part of the manual is organized as follows:
  ...
  - Chapter \@ref(cha:using-bugs-models) introduces more features of NIMBLE models that are useful for writing nimbleFunctions to use models, focusing on how to query model structure and carry out model calculations.  
  ...
  - Chapter \@ref(cha:progr-with-models) draws on the previous chapters to show how to write nimbleFunctions that work with models, or more generally that have a setup function for any purpose.  Typically a setup function queries model structure (Chapter \@ref(cha:using-bugs-models)) and may establish some `modelValues` or `nimbleList` data structures or configurations (Chapter \@ref(cha:data-structures)).  Then *run* functions written in the same way as simple nimbleFunctions (Chapter \@ref(cha:RCfunctions)) along with model operations (Chapter \@ref(cha:using-bugs-models)) define algorithm computations that can be compiled via C++.
