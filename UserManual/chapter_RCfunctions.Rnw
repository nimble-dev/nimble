...
Other R functions with numeric arguments and return value can be called during compiled execution by wrapping them as a \cd{nimbleRcall} (see \ref{sec:calling-R-code}).
