...
system("cat functionTableMatrixLong.md eigenSvdTableMatrixElements.md > tmp.md")
```
```{r, child = 'tmp.md'}
```
```{r echo=FALSE, include=FALSE}
...
above the observed times for uncensored entries (e.g., `Inf`). 
...
`censored[i] = 0` if `t[i]` $\le$ `c[i, 1]`

`censored[i] = m` if `c[i, m] < t[i]` $\le$ `c[i, m+1]` for $1 \leq m \leq M$
